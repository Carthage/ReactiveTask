	...
	return SignalProducer { observer, disposable in
		...
		SignalProducer(result: Pipe.create(queue, group) &&& Pipe.create(queue, group))
			.flatMap(.Merge) { stdoutPipe, stderrPipe -> SignalProducer<TaskEvent<NSData>, TaskError> in
				...
				return SignalProducer { observer, disposable in
					func startAggregating(producer: Pipe.ReadProducer, partial: (NSData) -> TaskEvent<NSData>) -> Pipe.ReadProducer {
						...
						producer.startWithSignal { signal, signalDisposable in
							...
							signal.observe(Observer(next: { data in
								observer.sendNext(partial(data))
					...
					let stdoutAggregated = startAggregating(stdoutProducer, partial: TaskEvent.StandardOutput)
					let stderrAggregated = startAggregating(stderrProducer, partial: TaskEvent.StandardError)
